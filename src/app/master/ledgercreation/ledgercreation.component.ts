...
export class LedgercreationComponent implements OnInit {
  ...
  public datasearch: Observable<any>;
  ...
  ngOnInit() {
    ...
    this.mode = "(List)";
    if (!this.Branch) {
      this.data = this._masterformservice.getledger("CW_1001");
    } else {
  ...
  onSubmit(form?: NgForm) {
    ...
    if (this.IsExistdata === true) {
      this.addToast("Cogwave Software", "Ledger already Exist ", "warning");
      return;
    }

    this._masterformservice.SaveLedger(form.value).subscribe(data => {
      if (data == true) {
        if (form.value.Id == "0") {
          this.addToast(
            "Cogwave Software",
            "Ledger Data Saved Sucessfully",
            "success"
          );
          form.reset({
            IsActive: "true",
          ...
          this.addToast(
            "Cogwave Software",
            "Ledger Data Updated Sucessfully",
            "success"
          );
          form.reset();
          this.mode = "(List)";
